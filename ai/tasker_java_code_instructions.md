...
*   **NO METHOD MODIFIERS IN ANONYMOUS CLASSES:** When overriding a method in an anonymous inner class (e.g., `new Runnable() { ... }` or `new OnClickListener() { ... }`), you **MUST** omit the access modifier (`public`, `protected`) and the return type (`void`, `String`, etc.). The method signature must start directly with the method name. This is a critical parser limitation in this version of BeanShell.
    *   **Correct Example:**
        ```java
        new Runnable() {
            run() { /* code here */ }
...
5.  **CODE COMMENTS:** You MUST use `/* ... */` for all comments. The commenting style MUST be concise and clean.
    *   **Default to Single-Line Blocks:** For most comments, write a short, single sentence on a single line. Example: `/* This is a concise, single-line block comment. */`
...
6.  **SHALLOW CODE STRUCTURE:** Prefer flat code structures over deep nesting. Use guard clauses (early returns) and early `continue`/`break` in loops.
    *   **GOOD Example (Flat):**
        `if (someObject == null) { return "Error: object is null"; }`
...
8.  **TASKER HELPER VARIABLE:** A pre-defined variable named `tasker` is always available. It provides methods to interact with Tasker and the system.
    ...
    *   **`boolean callTask(java.lang.String, java.util.HashMap<java.lang.String, java.lang.String>, int)`:** Runs a Tasker task by name with the given variables at the given priority, returning a boolean representing if the task was successfully started or not. Higher priority numbers win when several tasks are queued on Tasker's single work thread. It doesn't check if the task actually ran to completion and doesn't wait for it to actually run.
    ...
    *   **`void dismissSceneV2(java.lang.String)`:** Dismisses a specific Scene V2 by its Screen ID.
    ...
    *   **`android.graphics.Bitmap getImageBuffer()`:** Gets Tasker's current image buffer as a Bitmap. This is the image populated by the 'Load Image' action and modified by actions like 'Crop Image' and 'Resize Image'. Returns null if no image is currently loaded. Do NOT call recycle() on the returned Bitmap if later native Tasker actions (e.g. 'Save Image', 'Crop Image') will operate on this image, as it's the live buffer, not a copy.
    ...
    *   **`net.dinglisch.android.taskerm.ki$a getNotificationListener()`:** Returns an instance of Tasker's notification listener service if it's running. Otherwise it returns null.
    ...
    *   **`io.reactivex.Observable<com.joaomgcd.taskerm.action.java.SceneV2EventContext> getSceneV2Events()`:** Returns an RxJava2 Observable of SceneV2EventContext events emitted whenever any Scene V2 component is interacted with (clicked, toggled, swiped, etc.). Each SceneV2EventContext has fields: sceneName, screenId, elementId, elementPath, elementValue, interactionType, outputVariables. Use .subscribe() for async monitoring or .blockingFirst() to wait for the next event.
    *   **`io.reactivex.Observable<com.joaomgcd.taskerm.action.java.SceneV2EventContext> getSceneV2Events(java.lang.String)`:** Returns an Observable of SceneV2EventContext filtered to events from a specific Screen ID.
    *   **`io.reactivex.Observable<com.joaomgcd.taskerm.action.java.SceneV2EventContext> getSceneV2Events(java.lang.String, java.lang.String)`:** Returns an Observable of SceneV2EventContext filtered to events from a specific Screen ID and element ID.
    ...
    *   **`boolean hasImageBuffer()`:** Returns true if Tasker currently has an image loaded in its image buffer (i.e. a 'Load Image' action ran before this script).
    ...
    *   **`void setProfileEnabled(java.lang.String, boolean)`:** Enables or disables all Tasker profiles with the given name, exactly like the "Profile Status" action. Throws a JavaCodeException if no profile with that name exists.
    *   **`void showSceneV2(java.lang.String)`:** Shows a Scene V2 by name or inline JSON as fullscreen. The code continues immediately without waiting for user interaction.
    *   **`void showSceneV2(java.lang.String, java.lang.String)`:** Shows a Scene V2 with a custom Screen ID. The code continues immediately.
    *   **`void showSceneV2(java.lang.String, java.lang.String, java.lang.String)`:** Shows a Scene V2 with a custom Screen ID and display mode ('Fullscreen', 'Dialog', or 'Overlay'). The code continues immediately.
    *   **`void showSceneV2(java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String)`:** Shows a Scene V2 with overlay position. Position values can be in dp (e.g. '100') or percentage (e.g. '50%'). Use negative values to position from the opposite edge. The code continues immediately. Only applies when displayMode is 'Overlay'.
    *   **`void showSceneV2(java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String)`:** Shows a Scene V2 with overlay position and size. Values can be in dp or percentage. The code continues immediately. Only applies when displayMode is 'Overlay'.
    *   **`void showSceneV2(java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, boolean)`:** Shows a Scene V2 with overlay position, size, and touch blocking. 'blocking' controls whether the overlay blocks touch events behind it (true) or lets them pass through (false). The code continues immediately. Overlay params only apply when displayMode is 'Overlay'.
    *   **`void showSceneV2(java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, boolean, boolean, boolean)`:** Shows a Scene V2 with full overlay configuration. 'showOverEverything' uses the Accessibility Overlay. 'longClickRootToDismiss' adds a long-press dismiss gesture on the root element. The code continues immediately. Overlay params only apply when displayMode is 'Overlay'.
    *   **`void showSceneV2(java.lang.String, java.lang.String, java.lang.String, boolean, boolean)`:** Shows a Scene V2 with display mode and overlay options. 'showOverEverything' uses the Accessibility Overlay to show over other overlays and some system windows. 'longClickRootToDismiss' adds a long-press gesture on the root element to dismiss the overlay. Only apply when displayMode is 'Overlay'. The code continues immediately.
    *   **`io.reactivex.Single<java.util.Map<java.lang.String, java.util.List<java.lang.Object>>> showSceneV2ForResult(java.lang.String)`:** Shows a Scene V2 as fullscreen and returns a Single<Map<String,List<String>>> when the user dismisses the scene. The map's keys are variable names and values are lists of 1 or more values for each variable. Call .blockingGet() to wait for the result (only on a background thread) or .subscribe() for async handling.
    *   **`io.reactivex.Single<java.util.Map<java.lang.String, java.util.List<java.lang.Object>>> showSceneV2ForResult(java.lang.String, java.lang.String)`:** Shows a Scene V2 with a custom Screen ID and returns a Single<Map<String,List<String>>> when dismissed. The map's keys are variable names and values are lists of 1 or more values for each variable.
    *   **`io.reactivex.Single<java.util.Map<java.lang.String, java.util.List<java.lang.Object>>> showSceneV2ForResult(java.lang.String, java.lang.String, java.lang.String)`:** Shows a Scene V2 with a custom Screen ID and display mode, and returns a Single<Map<String,List<String>>> when dismissed. The map's keys are variable names and values are lists of 1 or more values for each variable.
    *   **`io.reactivex.Single<java.util.Map<java.lang.String, java.util.List<java.lang.Object>>> showSceneV2ForResult(java.lang.String, java.lang.String, java.lang.String, long)`:** Shows a Scene V2 and returns a Single<Map<String,List<String>>> when dismissed. The map's keys are variable names and values are lists of 1 or more values for each variable. Pass a timeout in milliseconds (0 for no timeout). The Single will error if the timeout is exceeded.
    *   **`io.reactivex.Single<java.util.Map<java.lang.String, java.util.List<java.lang.Object>>> showSceneV2ForResult(java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.Boolean, long)`:** Shows a Scene V2 with overlay params and returns a Single<Map<String,List<String>>> when dismissed. The map's keys are variable names and values are lists of 1 or more values for each variable. Overlay params only apply when displayMode is 'Overlay'.
    *   **`io.reactivex.Single<java.util.Map<java.lang.String, java.util.List<java.lang.Object>>> showSceneV2ForResult(java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.Boolean, boolean, boolean, long)`:** Shows a Scene V2 with full overlay configuration and returns a Single<Map<String,List<String>>> when dismissed. The map's keys are variable names and values are lists of 1 or more values for each variable. 'showOverEverything' uses the Accessibility Overlay. 'longClickRootToDismiss' adds a long-press dismiss gesture on the root element. Overlay params only apply when displayMode is 'Overlay'.
    ...
    *   **`void toggleProfileEnabled(java.lang.String)`:** Toggles the enabled state of all Tasker profiles with the given name, exactly like the "Profile Status" action set to Toggle. Throws a JavaCodeException if no profile with that name exists.
    *   **`void updateSceneV2(java.lang.String, java.lang.String, java.lang.String)`:** Updates a property of an element in all currently active Scene V2 screens. For example: updateSceneV2('myButton', 'text', 'Hello')
    *   **`void updateSceneV2(java.lang.String, java.lang.String, java.lang.String, java.lang.String)`:** Updates a property of an element in a specific Scene V2 screen. For example: updateSceneV2('myButton', 'text', 'Hello', 'myScreen')
    *   **`void updateSceneV2Overlay(java.lang.String, java.lang.String, java.lang.String)`:** Updates the position of an overlay Scene V2. Position values can be in dp (e.g. '100') or percentage (e.g. '50%').
    *   **`void updateSceneV2Overlay(java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String)`:** Updates the position and size of an overlay Scene V2. Values can be in dp or percentage.
    *   **`void updateSceneV2Overlay(java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.Boolean, int)`:** Updates the full overlay configuration including position, size, touch blocking, and transition animation duration in ms.
    *   **`com.joaomgcd.taskerm.action.java.SceneV2EventContext waitForSceneV2Event()`:** Blocks until the next Scene V2 component interaction from any scene and returns the SceneV2EventContext.
    *   **`com.joaomgcd.taskerm.action.java.SceneV2EventContext waitForSceneV2Event(java.lang.String)`:** Blocks until the next Scene V2 component interaction from the specified Screen ID and returns the SceneV2EventContext.
    *   **`com.joaomgcd.taskerm.action.java.SceneV2EventContext waitForSceneV2Event(java.lang.String, java.lang.String)`:** Blocks until the next Scene V2 component interaction from the specified Screen ID and element ID and returns the SceneV2EventContext.
    *   **`io.reactivex.Single<java.util.Map<java.lang.String, java.util.List<java.lang.Object>>> waitForSceneV2Result(java.lang.String)`:** Waits for a Scene V2 to be dismissed and returns a Single<Map<String,List<String>>>. The map's keys are variable names and values are lists of 1 or more values for each variable. The scene must already be active. Call .blockingGet() to wait synchronously (only on a background thread) or .subscribe() for async handling.
    *   **`io.reactivex.Single<java.util.Map<java.lang.String, java.util.List<java.lang.Object>>> waitForSceneV2Result(java.lang.String, long)`:** Waits for a Scene V2 to be dismissed with a timeout in milliseconds (0 for no timeout). Returns a Single<Map<String,List<String>>>. The map's keys are variable names and values are lists of 1 or more values for each variable.
    *   **`android.os.IBinder getShizukuService(java.lang.String)`:** Gets a system service via Shizuku for privileged operations. Throws a RuntimeException if Shizuku is not available.
    *   **`net.dinglisch.android.taskerm.ki$a getNotificationListener()`:** Returns an instance of Tasker's notification listener service if it's running. Otherwise it returns null.
    *   **`io.reactivex.Observable<com.joaomgcd.taskerm.helper.NotificationUpdate> getNotificationUpdates()`:** Returns an RxJava2 Observable of com.joaomgcd.taskerm.helper.NotificationUpdate events that you can subscribe to and then do stuff when each one arrives. Each NotificationUpdate object has a 'getCreated()' method that returns true if the notification was created, and false if it was removed. It also has a getStatusBarNotification() method that returns the android.service.notification.StatusBarNotification that was either posted or removed.
    *   **`io.reactivex.Observable<android.view.accessibility.AccessibilityEvent> getAccessibilityEvents()`:** Returns an RxJava2 Observable of Accessibility events that you can subscribe to and then do stuff when each one arrives
